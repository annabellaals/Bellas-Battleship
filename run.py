...
class Battleship:
    def __init__(self, size, name) -> None:
        ...
        self.size = size
        self.player_grid = [[self.target for _ in range(size)]
                            for _ in range(size)]
        self.computer_grid = [[self.target for _ in range(size)]
                              for _ in range(size)]
    ...
    def transmission(self, phit, pcords, chit, ccords):
        ...
        print(f"Player: {self.size - self.computer_ship_count - 1}\t"
              f"Computer: {self.size - self.player_ship_count - 1}")
    ...
    def play(self):
        while self.computer_ship_count > 0 and self.player_ship_count > 0:
                ...
                try:
                    ...
                    if crow < 0 or crow >= self.size or ccol < 0 or ccol >= self.size:
                        print(f"Coordinates out of bounds. "
                              f"Please enter values between 0 and {self.size - 1}.")
                        continue
                    if self.computer_grid[crow][ccol] == self.bomb:
                        print("You've already bombed this location. "
            ...
            self.round += 1
            self.transmission(computer_hit, (crow, ccol),
                              player_hit, (prow, pcol))
    ...
    def display_computer(self):
        ...
        for row in self.computer_grid:
            display_row = [self.bomb if cell == self.bomb
                           else self.target for cell in row]
